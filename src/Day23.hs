...
neighbors' :: Grid -> (Position, Dir) -> [(Position, Dir)]
neighbors' (grid, _) ((r, c), _) = [(p, d) | p <- [(r - 1, c), (r + 1, c), (r, c - 1), (r, c + 1)], Just d <- [M.lookup p grid]]
...
bfs :: M.Map (Position, Dir) [((Position, Dir), Int)] -> (Position, Dir) -> (Position, Dir) -> S.Set (Position, Dir) -> Int
bfs gs p t visited
    | p == t = 0
    | otherwise = case M.lookup p gs of
        Nothing -> 0
        Just ns -> maximum (map (\(p', l) -> l + bfs gs p' t (S.insert p' visited)) ns)

part1 :: String -> String
part1 input = show $ bfs gs starting target S.empty
  where
    (starting, target, grid) = parse input
    gs = graph grid target starting (neighbors grid) S.empty

part2 :: String -> String
part2 input = show $ bfs gs starting target S.empty
  where
    (starting, target, grid) = parse input
    gs = graph grid target starting (neighbors' grid) S.empty
